...
% Anchor reading: Mano Ch. 7 (state-table / sequence-counter method for
%                 hardwired control); Hamacher Ch. 7 (control signal
%                 conventions, control unit inputs/outputs);
%                 Stallings Ch. 16 (control unit design overview)
%
% Compile with: /compile-latex 06-hardwired-control
% =============================================================================
...
\begin{frame}{The Sequence Counter (\texttt{SC})}
  ...
  \end{block}
  \begin{center}
    % Diagram: SC register feeding a step decoder, producing one-hot T_i lines.
    % Coordinates: (x, y)
    ...
    \begin{tikzpicture}[scale=1.30, transform shape,
      ...
      \node[draw=primary-blue, fill=light-bg, rounded corners,
            minimum width=2.0cm, minimum height=1.0cm, align=center] (SC) at (0,0) {SC \\ (counter)};
      \node[draw=primary-gold, fill=white, rounded corners,
            minimum width=2.4cm, minimum height=1.6cm, align=center] (DEC) at (3.6,0) {Step\\decoder};
...
\begin{frame}{Two Decoders Feed the Control Matrix}
  \begin{center}
    ...
    % (T_j lines); both feed into a combinational "control matrix" block that
    % outputs the actual control signals.
    % Coordinates: (x, y)
    %   IR box at (0, 1.8); "Opcode decoder" at (3.2, 1.8); D_i label at (6.0, 2.3)
    %   SC box at (0, -1.8); "Step decoder" at (3.2, -1.8); T_j label at (6.0, -2.3)
    %   Control matrix box at (9.6, 0), tall, receiving both D_i and T_j as single
    %     unbroken paths (ODEC/SDEC -> elbow at x=8.0 -> MAT.west), one arrowhead each
    ...
    \begin{tikzpicture}[scale=0.90, transform shape,
      ...
      \node[draw=primary-gold, fill=white, rounded corners,
            minimum width=2.2cm, minimum height=1.0cm, align=center] (ODEC) at (3.2,1.8) {Opcode\\decoder};
      \draw[->, thick] (IR.east) -- (ODEC.west);
      \node[font=\scriptsize] at (6.0,2.3) {$D_1, D_2, \dots$};
      \node[draw=primary-blue, fill=light-bg, rounded corners,
            minimum width=1.6cm, minimum height=0.8cm, align=center] (SC) at (0,-1.8) {SC};
      \node[draw=primary-gold, fill=white, rounded corners,
            minimum width=2.2cm, minimum height=1.0cm, align=center] (SDEC) at (3.2,-1.8) {Step\\decoder};
      \draw[->, thick] (SC.east) -- (SDEC.west);
      \node[font=\scriptsize] at (6.0,-2.3) {$T_1, T_2, \dots$};
    ...
    \end{tikzpicture}
  \end{center}
  Every control signal is a Boolean function of the $D_i$ lines \emph{and} the
  $T_j$ lines together \cite{Mano1993_computer_system_architecture}.
\end{frame}
...
\begin{frame}{Full Hardwired Control Unit Block Diagram}
  \begin{center}
    ...
    % feeds back to clear/advance SC.
    % Coordinates: (x, y)
    ...
    \begin{tikzpicture}[scale=0.62, transform shape,
      ...
      \node[draw=primary-gold, fill=white, rounded corners,
            minimum width=2.0cm, minimum height=0.9cm, align=center] (ODEC) at (3.2,2.2) {Opcode dec.};
      \draw[->, thick] (IR.east) -- (ODEC.west);
      \node[draw=primary-blue, fill=light-bg, rounded corners,
      ...
      \node[draw=primary-gold, fill=white, rounded corners,
            minimum width=2.0cm, minimum height=0.9cm, align=center] (SDEC) at (3.2,-2.3) {Step dec.};
      \draw[->, thick] (SC.east) -- (SDEC.west);
      \node[draw=primary-blue, thick, fill=light-bg, rounded corners,
