...
%                 Mano Ch. 8 (single-bus datapath, RTL, addressing modes --
...
%                 Ch. 7 of this book is Microprogrammed Control, i.e. Lectures 06-07's
%                 material, not this lecture's)
%
...
\begin{frame}{The Single-Bus Datapath}
  \begin{center}
    ...
    \end{tikzpicture}
  \end{center}
  \footnotesize \cite{Mano1993_computer_system_architecture} --- cf.\ Fig.~5-4, p.129
  (registers and memory on the common bus system); the \texttt{Y}/\texttt{Z} buffer
  registers shown here are this course's pedagogical simplification of the
  ALU operand path.
\end{frame}
...
\begin{frame}{The Three-Bus Datapath}
  \begin{center}
    % Diagram: three-bus CPU datapath, redrawn to match Mano Fig. 8-2
    % ("Registers with common ALU", p.243) -- the register file feeds two
    % MUX-selected buses (Bus A, Bus B) into the ALU. Bus C is DATA ONLY:
    % it carries the ALU's result straight back into the register file.
    % The SELD decoder is a SEPARATE select path (dashed) that drives the
    % destination register's load-enable input -- it does not carry data,
    % so it is drawn apart from the Bus C data loop (fixes an earlier draft
    % that incorrectly routed Bus C's data through the decoder).
    % Coordinates: (x, y)
    %   Rfile box at (0,0), width 2.0, height 2.6 -- register file, 2 read ports + 1 write port
    %   MUXA at (2.4,0.9), width 1.0, height 1.0 -- selects one of 2 register
    %     lines onto Bus A (2 fan-in stubs at y=1.2/0.6, both within the box's
    %     0.4..1.4 vertical span)
    %   MUXB at (2.4,-0.9), width 1.0, height 1.0 -- mirror of MUXA for Bus B
    %   ALU box at (6.0,0), width 1.4, height 1.4
    %   Bus C (data, solid primary-blue): ALU.east -> (7.4,0) -> down to
    %     y=-2.6 -> left to x=0 -> up into Rfile.south (0,-1.3); label
    %     "write-back (data)" at (3.5,-3.1), 0.5cm clear of the y=-2.6 run
    %   DEC (SELD decoder, select only, no data) at (8.2,2.0), width 1.6,
    %     height 0.9 -- dashed primary-gold arrow left along y=2.0 (0.6cm
    %     clear of MUXA's top edge at y=1.4) into Rfile.north (0,1.3), a
    %     DIFFERENT entry point from Bus C's Rfile.south entry
  ...
  cycle (via MUX-selected \texttt{SELA}/\texttt{SELB}); \key{Bus C} carries the
  ALU's result directly back into the register file, while the \texttt{SELD}
  decoder (dashed) independently drives that register's load-enable line ---
  cf.\ Mano, Fig.~8-2, p.243 (``Registers with common ALU''); the same
  three-bus principle is also presented in Hamacher Ch.~7
...
\begin{frame}{Worked Trace: \texttt{R1} $\gets$ \texttt{R2 + R3} on Three Buses}
  \footnotesize
  \begin{tabular}{@{}cp{3.4cm}p{2.6cm}p{4.6cm}@{}}
    \toprule
    \textbf{Step} & \textbf{RTL} & \textbf{On the buses} & \textbf{What happens} \\
    \midrule
    $T_1$ & $R1 \gets [R2] + [R3]$ & $A{=}[R2]$, $B{=}[R3]$,
      $C{=}$result &
