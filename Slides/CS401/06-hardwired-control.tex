...
\begin{frame}{Where We Left Off (Week 5)}
  \emph{Last week: hardware datapaths, traced by hand.}
  \begin{itemize}
...
\begin{frame}{Roadmap: Three Questions, One Thread}
  \begin{enumerate}
    ...
    \end{enumerate}
  \vspace{0.4cm}
  \muted{\key{Running thread for today:} we reuse the \emph{same} instruction from
  Week 5, \texttt{R1} $\gets$ \texttt{R2} + \texttt{R3} on the single-bus datapath,
  and build the actual control unit that drives it --- so ``the hardware decides''
  becomes gates and equations, not hand-waving.}
\end{frame}
...
\begin{frame}{A Question We Skipped}
  \begin{block}{Socratic question}
    In Week 5's trace, $Y$ loaded from the bus at $T_1$ and $Z$ loaded from the
...
\begin{frame}{Notation: Register-Gating Control Signals}
  ...
  \begin{itemize}
    ...
    \item $T_2$: $R3_{\text{out}}$, ALU $\gets$ add, $Z_{\text{in}}$ \quad
      (R3 drives the bus, ALU adds \texttt{[Y]}, Z latches)
...
\begin{frame}{Two Philosophies for Building a Control Unit}
  \begin{center}
    ...
    \begin{tabular}{@{}p{3.4cm}p{4.9cm}p{4.9cm}@{}}
      \toprule
      & \textbf{Hardwired control} & \textbf{Microprogrammed control} \\
      \midrule
      Where the \,(opcode, step) $\to$ signal map lives &
      directly in logic gates &
      stored as microinstructions in a small memory \\
    ...
    \end{tabular}
  \end{center}
  \begin{exampleblock}{This week vs. next week}
...
\begin{frame}{The Sequence Counter (\texttt{SC})}
  ...
  \muted{Shown here as the general ISA-sized counter (4 bits, up to 16 steps);
  this course's worked trace only uses $T_1, T_2, T_3$, so its \texttt{SC} can
  be sized down to 2 bits --- see ``The Two Threads Meet.''}
\end{frame}
...
\begin{frame}{The Missing Piece: Opcode Decoding}
  ...
  \begin{itemize}
    ...
    \item New notation: the \key{$I$ flip-flop} --- captures \texttt{IR} bit 15,
      which marks an \emph{indirect} addressing instruction (Week 5). When
      $I = 1$, signals that resolve an address indirectly must behave
      differently, so the control matrix receives $I$ as an extra input.
...
\begin{frame}{The Two Threads Meet}
  \begin{center}
    ...
    \begin{tabular}{@{}p{3.4cm}p{4.6cm}p{4.6cm}@{}}
      ...
      \midrule
      Control steps for \texttt{R1} $\gets$ \texttt{R2+R3} & 3 ($T_1,T_2,T_3$) & 1 ($T_1$) \\
      \texttt{SC} width & 2 bits for this trace (general: up to 4 bits / 16 steps, cf.\ the SC frame) & barely needed \\
      Step-decoder outputs & 3 & 1 \\
      State-table rows for \texttt{ADD} & 3 & 1 \\
