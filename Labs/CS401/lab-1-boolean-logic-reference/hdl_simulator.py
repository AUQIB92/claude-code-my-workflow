...
chips only (sufficient for Lab 1; sequential chips from Lab 3 onward
...
bus indexing pin[i], and bus range slicing pin[lo..hi]. Does NOT
support: CLOCKED chips, true/false literal pins, internal buses
referenced before their width is established by a prior connection.
"""
...
def eval_chip(hdl_dir, name, input_bits):
    ...
    for part_name, conns in chip["parts"]:
        ...
        part_input_bits = {}
        for lhs, rhs in conns.items():
            if lhs in part_in_widths:
                ref_name, rng = _parse_signal_ref(rhs)
                part_input_bits[lhs] = get_slice(ref_name, rng)

        out_bits = eval_chip(hdl_dir, part_name, part_input_bits)

        for lhs, rhs in conns.items():
            if lhs in part_out_widths:
                ref_name, rng = _parse_signal_ref(rhs)
                produced = out_bits[lhs]
                if ref_name not in signals:
                    target_width = (rng[1] - rng[0] + 1) if rng else part_out_widths[lhs]
