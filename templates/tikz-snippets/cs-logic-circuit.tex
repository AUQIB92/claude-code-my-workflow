...
\begin{tikzpicture}
  ...
  % Routed with -| (horizontal-then-vertical) rather than a direct diagonal:
  % the input rails are spaced 0.8cm apart (y=1.4/0.6) but circuits.logic.US's
  % default 7pt symbol unit puts the gate's own input pins only ~0.205cm
  % apart, so a straight `--` here draws a visibly diagonal line into each
  % pin. `-|` always resolves squarely against the true anchor, regardless of
  % pin spacing, so this stays correct even if the symbol unit changes later.
