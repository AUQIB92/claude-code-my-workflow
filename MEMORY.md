...
[LEARN:textbook] `/index-textbook` must cross-check the syllabus's own topic phrase for the week being indexed against what's actually found in the book — not just index the chapter named by an existing (often unverified) citation and call it done. Chapter titles/numbers are frequently wrong across editions; the syllabus's specific nouns ("DMA", "IOP", "cache", "paging", ...) are the ground truth to search for in the extracted content itself. Caught 2026-08-11: CS401 Week 8's syllabus said "interrupts, polling, DMA, IOP," citing "Hamacher Ch. 4" — indexing found Ch. 4 wasn't even I/O content in the edition on disk, the real chapters were Ch. 3 + Ch. 7, and critically that even those correct chapters never use the terms "DMA" or "IOP" anywhere. A title-only check would have missed the DMA/IOP gap entirely. Now codified as a mandatory Step 0.5 in `.claude/skills/index-textbook/SKILL.md`: extract the syllabus's topic phrase, verify each named term's actual presence post-extraction, and record any gap in `index.md` so downstream lecture-creation knows to phrase that specific topic as general-treatment rather than assuming full coverage.
