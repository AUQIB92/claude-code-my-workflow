...
\begin{itemize}
  ...
  \item \textbf{Stallings, \textit{Computer Organization and Architecture}, Ch.~8} --- the source for the \emph{names} DMA and IOP. Note the 11th-ed.\ file's I/O chapter is Ch.~8 (not Ch.~7 as older editions numbered it; confirmed in the course KB). Not page-indexed; cite at chapter level only, never a page number.
  \item \textbf{Mano, \textit{Computer System Architecture}, Ch.~11} --- the isolated-I/O contrast (separate I/O address space and dedicated instructions). Chapter-level only; not page-indexed.
  \item \textbf{P\&H, \textit{Computer Organization and Design} (ARM ed.), Ch.~4} --- exceptions-as-generalized-interrupts framing. Chapter-level only; not page-indexed.
...
The speed mismatch that motivates everything: a CPU executes roughly one instruction per nanosecond ($10^9$ instructions/s); a keyboard produces roughly one keystroke every 200 milliseconds (about 5 keystrokes/s) --- a factor of roughly 200 million. If the CPU must read every keystroke, it spends that gap doing nothing unless it has a plan. The simplest plan is \textbf{memory-mapped I/O}: give each device a small set of registers --- a data register, a status register, a control register --- that occupy ordinary addresses in the CPU's address space, so the CPU reads and writes them with ordinary load/store instructions and needs no special I/O instructions. Six functions appear in every device interface: data register, status register, control register, address decoding, timing generation, and format conversion (Hamacher, 6th ed., \S3.1/\S3.1.1/\S7.4, pp.~96--97, 238--239). Concretely the three registers are the \textbf{data register} (the byte actually transferred, e.g.\ \texttt{KBD\_DATA}/\texttt{DISP\_DATA}), the \textbf{status register} (flags such as \texttt{KIN}/\texttt{DOUT}), and the \textbf{control register} (settings such as interrupt-enable bits).

\vspace{0.3em}
\noindent\textbf{Isolated vs.\ memory-mapped I/O.} The alternative to memory-mapped I/O is \textbf{isolated I/O}: a \emph{separate} address space reached only by dedicated I/O instructions, not ordinary loads/stores --- the classic IBM mainframe approach (Mano Ch.~11, chapter-level). Memory-mapped I/O needs no special instructions and reuses the CPU's existing load/store machinery, which is why the Hamacher examples assume it. The deck's comparison table (address space / instructions / simplicity / typical use) is worth showing once so students can name the choice explicitly; the rest of the lecture uses memory-mapped I/O.
...
\textbf{Exceptions --- the general form.} The interrupt mechanism is not special to I/O devices. An \textbf{exception} is any event that interrupts the normal flow of control --- an I/O interrupt, but also divide-by-zero, an illegal instruction, a page fault, or a timer tick --- and the CPU handles all of them with the same jump-to-handler machinery (save state, service, restore). Plainly, an interrupt is just the ``a device needs me'' kind of exception. This is worth one beat in class because it reframes the whole Act: the five-step scenario is not I/O-specific; it is how the CPU hands control to \emph{any} handler, and it is how the operating system later gets control back from user programs (Hamacher, 6th ed., \S3.2.6, p.116--119; P\&H Ch.~4). Ask: ``name three events that are not keystrokes but must still suspend the current program'' --- divide-by-zero, an illegal instruction, a page fault, a timer tick all qualify.
...
\textbf{From arbitration to DMA.} Once a device can become bus master, it can transfer data \emph{directly to/from memory}, with no CPU instruction touching the data; the CPU is interrupted once, when the whole block finishes. \textbf{Honest-phrasing note:} this 6th edition of Hamacher describes exactly this mechanism in \S7.3 but never names it ``DMA''; the term and standard treatment follow Stallings Ch.~8 (chapter-level, not page-indexed).

\textbf{DMA (general/standard treatment --- Stallings Ch.~8).} A \textbf{DMA controller} is given a source/destination address and a word count by the CPU, becomes bus master (via arbitration), and transfers the whole block directly to/from memory, interrupting the CPU only when the \emph{block} completes. Pro: CPU involvement drops from one ISR per word (1024 for the running 4096-byte block) to one setup $+$ one completion interrupt --- constant in block size. Con: the DMA controller and CPU now compete for the same memory bus (\textbf{cycle stealing}); extra hardware required. Plainly: the CPU says ``copy this block,'' goes back to work, and the DMA controller rings the bell when done.

\textbf{What's inside a DMA controller.} Four registers carry the whole transfer, all written once by the CPU at setup and owned by the controller from then on: \textbf{source/destination address} (where the data comes from and goes to), \textbf{word count} (words remaining, decremented after every word), and \textbf{control/status} (direction, mode, and the ``block done'' flag). The deck's Figure (DMA controller box with CPU, device, and memory around it) makes the layout concrete --- the DMA controller sits between CPU and device on the bus, and can become bus master to reach memory directly. \textbf{Board:} draw the CPU, the DMA controller with its four registers labelled, the device, and memory above; then narrate the six-step sequence below while pointing at the registers each step touches.

\textbf{The six-step DMA transfer sequence} (Stallings Ch.~8, general treatment):
\begin{enumerate}
  \item \textbf{Setup:} the CPU writes source address, destination address, word count, and direction into the DMA controller's registers.
  \item \textbf{Request:} the DMA controller asserts \texttt{BR} (bus request).
  \item \textbf{Grant:} the arbiter answers with \texttt{BG}; the DMA controller becomes bus master (Hamacher, 6th ed., \S7.3, p.237--238).
  \item \textbf{Transfer one word:} the controller reads one word from the device and writes it to memory (or the reverse), using one or two bus cycles.
  \item \textbf{Decrement:} it subtracts 1 from the word count; if still $>0$, return to step 2 for the next word.
  \item \textbf{Done:} when the count reaches 0, drop \texttt{BR} and raise a \emph{completion interrupt} --- the CPU's only involvement after setup.
\end{enumerate}
Compare with polling/interrupts: the CPU appears exactly twice, at the start and the end.

\textbf{The three DMA transfer modes} (how long the controller holds the bus; Stallings Ch.~8, general treatment):
\begin{itemize}
  \item \textbf{Burst (block) mode:} holds the bus for the \emph{entire} block --- all 1024 words in one continuous burst. Fastest, but the CPU is starved (cannot reach memory) for the whole burst.
  \item \textbf{Cycle-stealing (single) mode:} grabs the bus for \emph{one word}, releases it, re-requests. Slower per block, but the CPU keeps running between words.
  \item \textbf{Transparent (hidden) mode:} transfers \emph{only during CPU cycles that do not use the bus at all} --- the CPU never notices. Slowest, but zero effect on the CPU.
\end{itemize}
Plainly: burst = ``whole job now, don't bother me''; cycle stealing = ``one word, then back off''; transparent = ``only when you're not looking.'' The deck's three-row table (bus held / transfer speed / effect on CPU) is the summary artifact.

\textbf{IOP (general/standard treatment --- Stallings Ch.~8).} An \textbf{I/O processor} generalizes DMA one step further: a dedicated processor given a whole \emph{channel program} (several transfers, formats, simple decisions) executes it independently, interrupting the main CPU only when the entire program completes. DMA offloads \emph{moving}; an IOP offloads \emph{managing a sequence} --- the difference is how much decision-making is delegated away from the CPU. Plainly: DMA copies one block; an IOP runs a whole to-do list and reports when the list is done.

\begin{example}[ISR count vs.\ DMA cost --- second instance]
\label{ex:dmaboard}
\textbf{Board:} recompute the lecture's 4096-byte example for an 8192-byte block (4 bytes/word): interrupt-driven needs $8192/4=2048$ ISR invocations; DMA still needs exactly 2 CPU events (1 setup $+$ 1 completion interrupt). \textbf{Prediction trap before revealing:} ``guess how many ISR invocations an 8192-byte block needs, given the 4096-byte case needed 1024.'' (Exactly double --- 2048, linear in block size --- while DMA's 2 is unchanged, which is the entire argument for DMA.) Draw the cycle-stealing sketch while narrating: the DMA controller and CPU both want the same memory bus; a stolen cycle is a CPU memory access that has to wait. Then reprise the three modes on the same example: burst takes all 1024 transfer cycles in one continuous stretch; cycle stealing interleaves them with CPU accesses one word at a time (same total bus time); transparent uses only idle CPU cycles.
\end{example}
...
\begin{itemize}
  \item \textbf{Memory-mapped I/O} places device registers in the normal address space (the \textbf{isolated-I/O} alternative needs separate instructions and a separate space); \textbf{polling} tests a status flag (\texttt{KIN}/\texttt{DOUT}) in a loop and wastes CPU time in proportion to the device/CPU speed gap.
  \item \textbf{Interrupts} reverse the initiative: \texttt{PS.IE} and per-device enables (\texttt{KIRQ}/\texttt{DIRQ}) gate delivery; \textbf{vectored interrupts} dispatch in constant time via \texttt{IPENDING}/\texttt{IENABLE} and the interrupt-vector table; nesting lets a higher-priority request interrupt an ISR recursively, otherwise requests stay latched in \texttt{IPENDING}; an \textbf{exception} generalizes the same mechanism to non-I/O events (divide-by-zero, page faults, timer ticks).
  ...
  \item \textbf{Arbitration} (\texttt{BR}/\texttt{BG}) lets a device become bus master --- the basis for \textbf{DMA} (four controller registers; six-step transfer; burst / cycle-stealing / transparent modes; one completion interrupt) and \textbf{IOP} (whole channel programs; general treatment per Stallings Ch.~8).
\end{itemize}
