...
- Plain English: [a device pretends to be a few memory locations.]{.neutral}
...
[The interface also performs address decoding, timing, and format conversion --- the "six functions" of an interface circuit, Hamacher Ch. 7 §7.4, p. 238.]{.neutral}
...
[We use memory-mapped I/O for the rest of the deck --- it is what the Hamacher examples assume [@Hamacher2002_computer_organization]. The isolated form is the standard alternative (Mano Ch. 11).]{.neutral}
...
- Plain English: [the CPU keeps asking "are you ready yet?" in a loop until the answer is "yes," then does the work.]{.neutral}
...
[Each pass runs the same two-test instructions again --- the CPU is spinning, doing no useful work between keystrokes [@Hamacher2002_computer_organization].]{.neutral}
...
- Plain English: [the master switch is `IE`; each device also has its own little switch. Both must be on for the request to arrive.]{.neutral}
...
- Plain English: [polling-the-sources asks everyone "was it you?" in order; a vector table skips the asking and goes straight to the right ISR.]{.neutral}
...
[If the lower-priority device's request stays pending while the higher one is being serviced, it waits until interrupts are re-enabled --- or, with [**nesting**]{.hi-gold}, a higher-priority request can interrupt the ISR itself.]{.neutral}
...
- Plain English: [an interrupt is just one kind of exception --- the "a device needs me" kind.]{.neutral}
...
[A slow slave stretches the transfer by holding `Slave-ready` low for extra clock cycles --- the master simply waits, still synchronized to the shared clock [@Hamacher2002_computer_organization].]{.neutral}
...
[Each step waits for the previous one --- [**bus skew**]{.hi-gold} (wires arriving at slightly different times) never causes a false sample, unlike a synchronous bus's single fixed clock edge [@Hamacher2002_computer_organization].]{.neutral}
...
Any device connected via `BR`/`BG` can become bus **master**, not just the CPU --- the arbiter grants the bus to exactly one requester, by priority, each time [@Hamacher2002_computer_organization].

[Being bus master is how a device can talk to memory *directly* --- the key idea the next slide builds on.]{.neutral}
...
- Plain English: [the CPU says "copy this block," then goes back to work; the DMA controller does the copying and rings the bell when done.]{.neutral}
...
[The CPU writes all four registers once at setup; the DMA controller owns them from then on.]{.neutral}
...
[Compare with polling/interrupts: the CPU now appears exactly twice, at the start and the end.]{.neutral}
...
[Plain English: burst = "whole job now, don't bother me"; cycle stealing = "one word, then back off"; transparent = "only when you're not looking."]{.neutral}
...
- Plain English: [DMA copies one block; an IOP runs a whole to-do list of I/O jobs and reports back when the list is done.]{.neutral}
