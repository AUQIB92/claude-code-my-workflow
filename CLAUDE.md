...
- **Meta / workflow:** `/commit` `/learn` `/new-skill` `/checkpoint` `/context-status` `/deep-audit` `/coauthor-brief` `/triage-inbox` `/feature-discovery` `/repo-innovator` `/accreditation-autopilot` `/render-lecture-video`
