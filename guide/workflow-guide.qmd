...
format:
  html:
    ...
    include-after-body:
      text: |
        <!-- Cloudflare Web Analytics (injected at runtime, not as a literal src tag,
             so Quarto's self-contained bundler doesn't try to embed the remote script) -->
        <script>
        (function () {
          var s = document.createElement('script');
          s.type = 'module';
          s.src = 'https://static.cloudflareinsights.com/beacon.min.js';
          s.setAttribute('data-cf-beacon', '{"token": "a1fe2658b43b4757be5204e3c4812746"}');
          document.body.appendChild(s);
        })();
        </script>
        <!-- End Cloudflare Web Analytics -->
        <!-- GoatCounter Analytics (injected at runtime, same reason as above) -->
        <script>
        (function () {
          var s = document.createElement('script');
          s.async = true;
          s.src = '//gc.zgo.at/count.js';
          s.setAttribute('data-goatcounter', 'https://academic-workflow.goatcounter.com/count');
          document.body.appendChild(s);
        })();
        </script>
...
Claude Code ships with built-in skills beyond this template's 75: `/batch` orchestrates parallel refactoring across your codebase (using git worktrees for isolation), `/simplify` runs 3-agent code review and applies fixes, and `/debug` helps troubleshoot sessions. These complement the academic skills above.
:::
...
The guide includes 75 [skills](#skills---reusable-slash-commands) for common academic tasks. But if you have repetitive workflows specific to your domain, you can create your own.
...
| `/feature-discovery` | `.claude/skills/feature-discovery/` | Analyze any software repository — 5-persona fan-out (Architect/Staff Engineer/PM/Security-Quality/OSS Maintainer) over evidence-cited findings — into a prioritized, scored "what to build next" report, with roadmap/spec/GitHub-issue-draft modes (v1.0) |
| `/repo-innovator` | `.claude/skills/repo-innovator/` | Discover the 2-3 innovations that could make a repository category-defining — deep repo intelligence, then a diverge/combine/contrarian/converge pipeline scored and cut via a 7-question "No. 1 Test"; refuses generic features unless strategically justified (v1.0) |
| `/accreditation-autopilot` | `.claude/skills/accreditation-autopilot/` | Roll up NBA/AICTE CO-PO-PSO accreditation data across every course in a program into one program-level Self-Assessment Report, filling any missing per-course draft via `/accreditation` first (v1.0) |
| `/render-lecture-video` | `.claude/skills/render-lecture-video/` | Render an already-authored step-diagram sequence into a silent, captioned animated GIF — captions copied verbatim from the source `\caption{}` text, never LLM-generated (v1.0) |
