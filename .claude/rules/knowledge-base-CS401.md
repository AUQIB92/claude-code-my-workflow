...
| Stallings, *Computer Organization and Architecture* — **11th ed. on disk** (Stallings2015; directory name and bib key are a misnomer kept for citation stability) | `master_supporting_docs/CS401/supporting_books/Stallings2015/index.md` — **built 2026-08-17, all 21 chapters + App. A-G at chapter level, with §5.2 broken out to sub-subsection depth for Week 9.** 1111-page text-layer PDF; index built from the embedded 227-entry PDF outline joined against the book's own Contents (PDF pp. 7-12). Edition confirmed from PDF metadata title: **"Eleventh Edition."** **Pagination: this is a reflowed web-capture ebook with no printed page numbers in the body text layer.** Printed pages come from the Contents and exist only for chapters and numbered sections; the PDF↔printed offset is non-constant and *grows* through the book (Ch. 4 +37 · Ch. 5 +41 · Ch. 8 +60 · Ch. 19 +136) — **never apply a fixed offset**. Cite as `Stallings 11e, §5.2, p.~147 (PDF 188)`. **Chapter renumbering vs. the 10th ed. is now fully mapped** — the syllabus's Week 3/10/11/12 Stallings citations are wrong (Ch. 9→**11**, Ch. 8,6→**9,7**, Ch. 17-18→**20 §20.1, 16 §16.4**, Ch. 13→**17**); only Week 9 was corrected in this pass. **Verified absent from the whole book: Belady's anomaly (0 hits), Optimal/OPT replacement (0), clock/second-chance (0); Flynn's taxonomy is ~1 page in §20.1; "associative memory" has no standalone treatment (closest is CAM, §5.2).** | Weeks 1-12 — chapter-level for all, page-citable throughout; primary anchor for Week 9 (Ch. 4, **Ch. 5**, Ch. 6 §6.1) and Week 10 (Ch. 9 §9.3-9.5 VM, Ch. 7 storage) | **Yes, page-cited (chapter + numbered-section printed pages authoritative from Contents; sub-subsection printed pages interpolated and marked `~` — cite the PDF page for those). Do NOT cite this book for Belady/OPT/Flynn — use Hamacher or P&H.** |
...
## Course Outcomes & Assessment Blueprint

**Source:** `syllabi/CS401.md`'s Learning Objectives (numbered here for the first time; instantiates the stub at `knowledge-base-template.md` §6). Generated by `/accreditation` — see `Accreditation/CS401/co-po-attainment.tex` for the full CO-PO-PSO correlation matrix and attainment methodology. Bloom levels are a first-pass draft from each objective's stated verb, not verified pedagogical practice; two-verb objectives are flagged with both levels rather than silently collapsed to one.

| CO | Statement | Bloom level | Lectures | Assessed in |
|----|-----------|------------|----------|-------------|
| CO1 | Compute and compare system performance using the basic performance equation, clock rate, and Amdahl's Law | Apply (flag: "compare" nudges toward Analyze) | Week 1 | Class Test 1, Assignment 1, End-sem |
| CO2 | Represent and manipulate numbers in fixed-point and IEEE754 floating-point formats, and perform arithmetic/logical operations in hardware | Apply | Weeks 2-3 | Class Test 1, End-sem, Lab 1-2 |
| CO3 | Describe instruction sets, addressing modes, and CPU register/bus organization, and trace an instruction cycle | Understand/Apply (flag: "describe" vs. "trace" are different levels) | Weeks 4-5 | Class Test 2, Assignment 2, End-sem, Lab 4 |
| CO4 | Design and contrast hardwired and microprogrammed control units for a given instruction set | Create (flag: "design" vs. "contrast" are different levels) | Weeks 6-7 | Class Test 2, End-sem |
| CO5 | Analyze I/O techniques (interrupt, polling, DMA, IOP) and select the appropriate technique for a given latency/throughput requirement | Analyze | Week 8 | Class Test 3, Assignment 3, End-sem |
| CO6 | Evaluate memory hierarchy performance, including cache mapping/replacement policy choice and virtual memory (paging/segmentation) design | Evaluate (flag: "design" component nudges toward Create) | Weeks 9-10 | Class Test 3, End-sem |
| CO7 | Compare Flynn's taxonomy classes and identify/resolve pipeline hazards in a pipelined instruction stream | Analyze | Week 11 | Assignment 4, End-sem |
| CO8 | Contrast RISC and CISC architectural philosophies and justify a design choice for a given workload | Evaluate (flag: "contrast" vs. "justify" are different levels) | Week 12 | End-sem |

<!-- OBE/NBA-facing. CO statements are copied verbatim from syllabi/CS401.md's
     Learning Objectives -- do not paraphrase them freely afterwards. If the
     syllabus's objectives change, re-run /accreditation rather than hand-
     editing this table, so the CO-PO matrix in Accreditation/CS401/ stays
     in sync with what's actually taught. -->
...
| 2 — Boolean Arithmetic & the ALU | 3-4 | HalfAdder, FullAdder, Add16, Inc16, ALU (18 ops) | Week 2 (`02-number-systems-arithmetic` — two's complement, ripple-carry adders, ALU) | Built |
